...
#head(df.comp)
#pos <- df.comp$Pos %>% as.character() %>% strsplit(",") %>% sapply("[[",1)
#df.comp$Pos <- as.factor(pos)


## Start to construct the nearest neighbors structure
#-------------------------------------------------------------
...
## loop over all random_players and check the average prediction error
...
## function to execute and clean model tests
